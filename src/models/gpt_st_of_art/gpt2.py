...
class AttentionLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config, name=None, trainable=True, initializer_range=0.02):
        ...
        self.self_attention = SelfAttention(num_attention_heads=config["model"]["config"]["num_heads"],
                                            size_per_head=config["model"]["config"]["embed_dim"] // config["model"]["config"]["num_heads"],
                                            initializer_range=initializer_range,
                                            name="self"
                                            )
        self.projection = tf.keras.layers.Dense(units=config["model"]["config"]["embed_dim"],
                                                kernel_initializer=tf.random_normal_initializer(stddev=initializer_range),
                                                name="projection")
...
class Block(tf.keras.layers.Layer):
    ...
    def __init__(self, config, trainable=True, initializer_range=0.02, name=None):
        ...
        self.mlp = MultiLayerPerceptron(activation_fn=gelu,
                                        embedding_size=config["model"]["config"]["embed_dim"],
                                        perceptron_size=4 * config["model"]["config"]["embed_dim"],
                                        initializer_range=initializer_range,
                                        name="mlp")
...
class Transformer(tf.keras.Model):
    ...
    def __init__(self, config, trainable=True, name=None):
        ...
        self.blocks = []
        self.blocks_num = config["model"]["config"]["layers"]
...
class GPT2(tf.keras.Model):
    ...
    def __init__(self, config, name=None, trainable=True, dtype=None):
        ...
        #     max_position_length=config["max_sequence_len"],
        #     name="embedding",
        #     dtype=dtype
        # )
        self.embedding = TokenAndPositionEmbedding(
            ...
            vocab_size=config["vocab_size"],
            embed_dim=config["model"]["config"]["embed_dim"]
        )
